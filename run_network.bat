...
start "Blockchain Node 8000" cmd /c "blockchain_node.exe --host 127.0.0.1 --port 8000 --type full --api-port 8080 --cors-enabled --api-allow-origin "*" %CLEAN_FLAG%"

REM Wait for the first node to start up
timeout /t 3 /nobreak

echo Starting node on port 8001 (Full Node) with API on port 8081...
start "Blockchain Node 8001" cmd /c "blockchain_node.exe --host 127.0.0.1 --port 8001 --type full --api-port 8081 --cors-enabled --api-allow-origin "*" %CLEAN_FLAG%"

REM Wait before starting the next node
timeout /t 4 /nobreak

echo Starting node on port 8005 (Full Node) with API on port 8085...
start "Blockchain Node 8005" cmd /c "blockchain_node.exe --host 127.0.0.1 --port 8005 --type full --api-port 8085 --cors-enabled --api-allow-origin "*" %CLEAN_FLAG%"

REM Wait before starting the next node
timeout /t 5 /nobreak

echo Starting node on port 8010 (Wallet Node) with API on port 8090...
start "Blockchain Node 8010" cmd /c "blockchain_node.exe --host 127.0.0.1 --port 8010 --type wallet --api-port 8090 --cors-enabled --api-allow-origin "*" %CLEAN_FLAG%"
